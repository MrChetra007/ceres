...
- **Database migrations:** every schema change is an incremental SQL migration in `migrations/`, named `NNN_short_slug.sql` and applied in number order in the Supabase SQL editor — from `000_baseline_schema.sql` (initial profile/field/link-code/alert tables) up to the latest `013_subscription_self_service_rpc.sql`. Zero-padded ordering = apply order; migrations are additive and use `CREATE OR REPLACE` where they supersede earlier logic (e.g. `012` redefines `enforce_aoi_limit`), so re-runs of earlier numbers are safe. Recurring audit/validation scripts are NOT migrations and live separately in `migrations/audits/` (e.g. `validation-86_dedup_audit.sql`). A new migration continues the sequence at `014_…`.
...
- **Subscriptions & billing UI:** three tiers (Free / Individual $5-mo / Co-op from $39-mo) built on the `subscription_*` profile columns + `billing_events` audit table from `migrations/012_subscription_tiers.sql`. Public pricing cards on the landing page **and** a dedicated `/pricing` route (signed-in users can reach it from paywalls); tier display + usage (AOI count, total field hectares) in a Plan & billing modal and a topbar tier badge; friendly upgrade **paywalls** for AOI cap, hectare cap (UI-only, backend enforcement pending) and locked Consult AI; payment history (humanized `billing_events` rows); cancel flow that keeps tier access until `subscription_renews_at`, then free. Checkout is a **placeholder** (`upgrade_my_subscription` RPC grants the tier with no charge, clearly marked temporary) — the real ABA PayWay Purchase API swaps in at one isolated `initiatePayment()` spot (`migrations/013_subscription_self_service_rpc.sql` provides the grant/cancel RPCs).
