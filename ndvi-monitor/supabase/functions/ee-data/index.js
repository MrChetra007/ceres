...
function applyIndex(img, index, name) {
    if (index === "savi") {
        const L = 0.5;
        return img
...
async function buildMaskedComposite(geom, start, end, index) {
...
// d may be a plain timestamp, a JS Date, or an ee.Date — only the last
        // case is a computed value that must be evaluated before Date consumes it.
        const iso = async (d) => {
            if (typeof d === "number")
                return tsToISO(d);
            if (d && typeof d.getTime === "function")
                return tsToISO(d.getTime());
...
async function initEE() {
    ...
    await new Promise((resolve, reject) => {
        ee.data.authenticateViaPrivateKey(EE_KEY, () => ee.initialize(null, null, () => resolve(), (e) => reject(new Error(e))), (e) => reject(new Error(e)));
    });
    console.log(`[ee-data] EE session initialized in ${Math.round(performance.now() - t0)}ms`);
}
let eeReady = null;
let eeReadyAt = 0;
...
function evaluate(obj) {
    return new Promise((resolve, reject) => obj.evaluate((res, err) => err ? reject(new Error(err)) : resolve(res)));
}
function getMapUrl(obj, vis) {
    return new Promise((resolve, reject) => obj.getMap(vis, (mapId, err) => err || !mapId || !mapId.urlFormat
        ? reject(new Error(String(err || "getMap failed")))
        : resolve(mapId.urlFormat)));
}
function toEeGeometry(geojson) {
...
function tileIsClosed(year, month) {
...
async function getRadarVegetationIndex(geom, startDate, endDate) {
    const s1 = ee
    ...
    const url = await getMapUrl(rvi, VIS.rvi);
    return { count, url };
}
...
async function getRadarRviValue(geom, startDate, endDate) {
    const s1 = ee
        .ImageCollection("COPERNICUS/S1_GRD")
        .filterBounds(geom)
        .filterDate(startDate, endDate)
    ...
    const vhLinear = ee.Image(10).pow(composite.select("VH").divide(10));
    const rvi = vhLinear.multiply(4).divide(vvLinear.add(vhLinear)).rename("RVI");
    const result = await evaluate(rvi.reduceRegion({
...
function stageNameAsOf(sceneDate, plantingDate) {
    if (!plantingDate)
        return null;
    const days = Math.floor((new Date(sceneDate + "T00:00:00Z").getTime() - new Date(plantingDate).getTime()) / 86400000);
    return stageNameForDayCount(days >= 0 ? days : null);
}
async function actionGetIndexTile(payload) {
    ...
    // A SCENE-DATED request (an observation clicked in the strip) is never
    // served from this month cache: the rows are keyed on (index, year, month,
    // geometry_hash, mode) with no date column, so only month-scrub requests
    ...
    if (widenedCount === 0) {
        ...
        const lastValidDate = tsToISO(await evaluate(bestScene.get("system:time_start")));
        let url = null;
        try {
...
async function actionGetTrueColorScene(payload) {
    const geom = toEeGeometry(payload.geometry);
    ...
    const all = ee
        ...
        .filterBounds(geom)
        .filterDate(start, end);
    const list = all.map((img) => ee.Feature(null, {
    ...
    let chosen = null;
    if (payload.sceneDate) {
        const sameDate = scenes.filter((s) => s.date === payload.sceneDate);
        if (sameDate.length) {
            chosen = sameDate.reduce((a, b) => b.cloudPct < a.cloudPct ? b : a);
        }
    }
    if (!chosen) {
        chosen = scenes.reduce((a, b) => b.cloudPct < a.cloudPct ? b : a);
    }
...
async function actionGetLatestTrueColor(payload) {
    const geom = toEeGeometry(payload.geometry);
...
function zoneBuckets(kind) {
    ...
    for (let i = 1; i < 10; i++)
        arr.push({ lo: i * 0.1, hi: (i + 1) * 0.1 });
    return arr;
}
async function reduceZoneBands(indexImage, buckets, geom) {
    ...
    const result = await evaluate(ee.Image(parts).reduceRegion({
...
async function actionGetZoneBreakdown(payload) {
...
async function actionGetIndexTimeSeries(payload) {
    const index = payload.index && BANDS[payload.index] ? payload.index : "ndvi";
    const months = payload.months || [];
    if (!months.length)
        return { points: [] };
    const geom = payload.lat != null && payload.lng != null
    ...
    const points = ((result && result.features) || []).map((f) => ({
        date: f.properties.date,
        cloudPct: f.properties.cloudPct,
...
// signal.
async function actionGetRviTimeSeries(payload) {
    const months = payload.months || [];
    if (!months.length)
        return { points: [] };
    const geom = payload.lat != null && payload.lng != null
...
async function actionDetectPlantingDate(payload) {
    const geom = toEeGeometry(payload.geometry);
    const end = ee.Date(Date.now());
...
async function actionGetDryMonths(payload) {
    const geom = toEeGeometry(payload.geometry);
    const months = payload.months || [];
    if (!months.length)
        return { dryMonths: [] };
    const monthFC = ee.FeatureCollection(months.map((m, i) => ee.Feature(null, { idx: i, year: m.year, month: m.month })));
...
async function computeNdviOverWindow(geom, days) {
    const end = ee.Date(Date.now());
    ...
    const result = await evaluate(img.reduceRegion({
        reducer: ee.Reducer.mean(),
        geometry: geom,
        scale: 10,
        maxPixels: 1e9,
...
async function computeIndexOverWindow(geom, index, days, month) {
    if (month) {
...
async function reduceIndexMean(geom, index, start, end) {
    const collection = s2Collection(geom, start, end);
    const count = await evaluate(collection.size());
    if (!count)
        return null;
...
function normalizeToUnitRange(index, value) {
    if (value == null)
        return null;
    const v = VIS[index];
...
async function actionGetFieldStatus(payload) {
    const geom = toEeGeometry(payload.geometry);
...
async function actionGetFieldHealthScore(payload) {
    const geom = toEeGeometry(payload.geometry);
    ...
    const weights = {};
    if (present.length) {
        const presentWeight = present.reduce((sum, i) => sum + (baseWeights[i] ?? 0), 0);
        const scale = presentWeight > 0 ? 1 / presentWeight : 1;
        for (const i of present)
            weights[i] = (baseWeights[i] ?? 0) * scale;
    }
    const score = Math.round(Object.entries(weights).reduce((sum, [i, w]) => sum + (normalized[i] ?? 0) * w, 0) * 100);
...
async function actionGetRecentIndexValue(payload) {
    const index = payload.index && BANDS[payload.index] ? payload.index : "ndvi";
    const name = index.toUpperCase();
    const geom = toEeGeometry(payload.geometry);
    ...
    const dayStart = f && !isNaN(f.getTime())
    ...
    const count = await evaluate(clean.size());
    if (count === 0)
        return { count: 0, value: null, date: null, cloudBlocked };
    ...
    const result = await evaluate(idxImg.reduceRegion({
        reducer: ee.Reducer.mean(),
        geometry: geom,
        scale: 10,
        maxPixels: 1e9,
...
async function actionGetObservations(payload) {
    const geom = toEeGeometry(payload.geometry);
...
async function actionGetAllFieldStatuses(payload) {
    const index = payload.index && BANDS[payload.index] ? payload.index : "ndvi";
    ...
    if (!incoming.length)
        return { statuses: [] };
    const fc = ee.FeatureCollection(incoming.map((f) => ee.Feature(toEeGeometry(f.geometry), { fid: String(f.id) })));
    ...
    const statuses = fc.map((ft) => {
        ...
        const dayStart = ee.Date(freshestTs.divide(86400000).floor().multiply(86400000));
    ...
    const rows = ((result && result.features) || []).map((f) => {
...
async function actionGetAllFieldTrends(payload) {
    const index = payload.index && BANDS[payload.index] ? payload.index : "ndvi";
    const name = index.toUpperCase();
    const months = payload.months || [];
    const incoming = Array.isArray(payload.fields) ? payload.fields : [];
    if (!incoming.length || !months.length)
        return { trends: [] };
    ...
    const now = Date.now();
    const closed = months.filter((m) => Date.UTC(m.year, m.month, 1) <= now);
    const open = months.filter((m) => Date.UTC(m.year, m.month, 1) > now);
    const closedKeys = new Set(closed.map((m) => monthKey(m.year, m.month)));
    ...
    try {
        ...
        const { data } = await supabase
            ...
            .eq("index", index)
            .in("field_id", incoming.map((f) => f.id));
        for (const row of data || []) {
            if (!row.is_closed_period)
                continue;
    ...
    for (const m of closed) {
        ...
        const anyFieldCached = incoming.some((f) => cacheMap.has(`${f.id}|${mk}`));
        if (!anyFieldCached)
            missing.add(mk);
    }
    for (const m of open)
        missing.add(monthKey(m.year, m.month));
    const computeMonths = months
        .filter((m) => missing.has(monthKey(m.year, m.month)))
    ...
    if (computeMonths.length) {
        ...
        const startDate = ee.Date.fromYMD(computeMonths[0].year, computeMonths[0].month, 1);
        const last = computeMonths[computeMonths.length - 1];
        const endDate = ee.Date.fromYMD(last.year, last.month, 1).advance(1, "month");
        const fc = ee.FeatureCollection(incoming.map((f) => ee.Feature(toEeGeometry(f.geometry), { fid: String(f.id) })));
        const trends = fc.map((ft) => {
            const geom = ee.Feature(ft).geometry();
            const all = ee
                .ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
                .filterBounds(geom)
        ...
        const rows = ((result && result.features) || []).map((f) => ({
            id: f.properties.fid,
            points: ((f.properties.points || {}).features || []).map((p) => p.properties),
        }));
        ...
        for (const f of incoming) {
            const buckets = freshByField.get(String(f.id)) ?? new Map();
    ...
    const trends = incoming.map((f) => {
        ...
        for (const m of months) {
            ...
            if (cached)
                byMonth.set(mk, cached);
        }
        const freshBuckets = freshByField.get(idStr);
        if (freshBuckets) {
            for (const [mk, pts] of freshBuckets)
                byMonth.set(mk, pts);
        }
        const points = months
            .flatMap((m) => byMonth.get(monthKey(m.year, m.month)) || [])
            .sort((a, b) => a.date.localeCompare(b.date));
...
async function actionGetFieldBundle(payload) {
...
Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log(`[ee-data] ${body.action} ok in ${Math.round(performance.now() - started)}ms`);
        return jsonResponse({ ok: true, ...data }, 200, corsHeaders);
    }
    catch (e) {
        console.error(`[ee-data] ${body.action} failed after ${Math.round(performance.now() - started)}ms:`, e);
        const status = typeof e?.code === "number" ? e.code : 500;
        const error = e?.message === "invalid_geometry"
            ? "invalid_geometry"
            : String(e?.message || e);
