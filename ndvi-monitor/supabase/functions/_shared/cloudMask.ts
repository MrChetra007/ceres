 ...
 */
export function addCloudProbability(collection: any, ee: any) {
  const s2c = ee.ImageCollection("COPERNICUS/S2_CLOUD_PROBABILITY");
  const withProb = collection.map((img: any) => {
 ...
 */
export function validPixelMask(scene: any, ee: any) {
 ...
 */
export async function validPixelFraction(
  image: any,
  band: string,
  geometry: any,
  scale: number,
