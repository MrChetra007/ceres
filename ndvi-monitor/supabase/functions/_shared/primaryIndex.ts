...
export function primaryIndexForStage(stageName: string | null): string {
  if (stageName && EARLY_STAGES.has(stageName)) return "savi";
  return "ndvi";
}
...
export function primaryIndexReasonKey(stageName: string | null): string | null {
  if (stageName && EARLY_STAGES.has(stageName)) return "aim.reason_savi_early";
  return null;
}
...
export function stageNameForDayCount(dayCount: number | null): string | null {
