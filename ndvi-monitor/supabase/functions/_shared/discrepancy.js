...
export function detectDiscrepancy(raw, ctx) {
    ...
    if (raw.ndvi != null &&
        ...
        return {
            ...
            message: "Canopy looks healthy, but soil moisture (LSWI) is low — consider irrigation soon.",
        };
    }
    ...
    if (ctx && ctx.score != null) {
        ...
        if (comp.label === "dead" || comp.label === "unhealthy") {
            ...
            const headBand = head != null ? translateIndexValue(ctx.primaryIndex, head) : null;
            if (headBand && (headBand.label === "healthy" || headBand.label === "moderate")) {
                if (raw.lswi != null && raw.lswi < 0.15) {
                    return {
                        messageKey: "aim.disc_lswi_pulls",
                        message: "The headline index reads healthy, but low soil moisture (LSWI) is pulling the overall score down.",
                    };
                }
                if (raw.evi != null && raw.evi < 0.5) {
                    return {
                        messageKey: "aim.disc_evi_pulls",
                        message: "The headline index reads healthy, but lower vegetation vigour (EVI) is pulling the overall score down.",
                    };
                }
                return {
                    messageKey: "aim.disc_mixed",
                    message: "The headline index reads healthy, but a weaker secondary index is pulling the blended score down.",
                };
            }
        }
    }
