...
function computeNdviOverWindow(geom, days) {
    return new Promise((resolve, reject) => {
...
function getRadarVegetationIndex(geom) {
    return new Promise((resolve, reject) => {
...
async function getFieldReading(geojson) {
    ...
    const rvi = await getRadarVegetationIndex(geom).catch((e) => {
        console.warn(`Radar fallback failed: ${e instanceof Error ? e.message : String(e)}`);
        return null;
    });
    if (rvi !== null) {
...
function getRainfallMm(geojson) {
    return new Promise((resolve, reject) => {
        const geom = toEeGeometry(geojson);
        const end = ee.Date(Date.now());
        ...
        ee.ImageCollection("UCSB-CHG/CHIRPS/DAILY")
            ...
            if (err)
                ...
                resolve(result?.precipitation ?? 0);
        });
    });
}
...
function getLswiForGeometry(geojson) {
    return new Promise((resolve, reject) => {
        const geom = toEeGeometry(geojson);
        const end = ee.Date(Date.now());
...
function getNdviTrendPct(geojson) {
    return new Promise((resolve, reject) => {
        const geom = toEeGeometry(geojson);
        const end = ee.Date(Date.now());
        const start = end.advance(-90, "day");
        ...
        series
            ...
            for (let i = pts.length - 2; i >= 0; i--) {
                const d = pts[i];
                if (d.value != null) {
                    const diffDays = (new Date(pts[pts.length - 1].date).getTime() -
...
function buildAlertMessage(fieldName, status, stage, ndvi, rainfall, weather, lang) {
        ...
        ? status === "stressed"
            ...
            : status === "below_expected"
                ...
                : status === "healthy"
                    ...
                    : status.replace("_", " ")
        : status.replace("_", " ");
    const wxText = weather?.precipitation_probability == null
        ? ""
        : lang === "km"
...
function buildRadarAlertMessage(fieldName, rvi, lang) {
...
async function sendTelegram(chatId, text) {
    try {
        ...
        if (!res.ok) {
            ...
            console.error(`Telegram send failed for chat ${chatId}: ${res.status} ${body}`);
            return false;
        }
        return true;
    }
...
function forecastContextLine(weather) {
    if (!weather || !weather.forecast_days?.length)
        return "weather forecast: n/a";
    const days = weather.forecast_days
        .map((d) => `${d.date.split("T")[0]}: rain ${d.rainPct == null ? "n/a" : `${d.rainPct}%`}, max ${d.tMax == null ? "n/a" : `${Math.round(d.tMax)}°C`}, min ${d.tMin == null ? "n/a" : `${Math.round(d.tMin)}°C`}`)
        .join("; ");
    return `weather forecast (next ~5 days): ${days}`;
}
async function buildAdvisoryPrompt(field, ndvi, status, stage, rainfall, weather, lang) {
...
Deno.serve(async (_req) => {
    try {
        ...
        const { data: fields, error } = await supabase
            .from("fields")
            .select("id, name, geojson, planting_date, crop_english, centroid_lat, centroid_lng, owner_id, profiles!inner(telegram_chat_id, preferred_language)")
        ...
        for (const field of fields || []) {
            try {
                const chatId = field.profiles?.telegram_chat_id;
                const lang = field.profiles?.preferred_language || "en";
                if (!chatId)
                    continue;
                ...
                const useGeneric = !!(field && field.crop_english) && field.crop_english !== "rice";
                ...
                try {
                    ...
                    if (lat != null && lng != null) {
                        ...
                        console.warn(`No centroid for field ${field.name} (${field.id}) — skipping weather forecast`);
                    }
                }
                catch (e) {
                    console.warn(`Weather fetch failed for field ${field.name}: ${e instanceof Error ? e.message : String(e)} — continuing with stress data only`);
                }
                const prompt = await buildAdvisoryPrompt(field, ndvi, status, stage, rainfall, weather, lang);
                const result = await generateExplanation(prompt);
                let modelUsed = null;
                let message;
                if (result && !result.truncated) {
                    modelUsed = result.model;
                    message = safeText(result.text, buildAlertMessage(field.name, status, stage, ndvi, rainfall, weather, lang));
                }
                else {
                    if (result?.truncated) {
                        modelUsed = result.model;
                    ...
                    message = buildAlertMessage(field.name, status, stage, ndvi, rainfall, weather, lang);
                }
                ...
                console.error(`Failed processing field ${field.name} (${field.id}):`, fieldErr);
                results.push({
                    field: field.name,
                    status: "error",
                    sent: false,
