...
- **Rest of Phase 8 pending — planned architecture (Supabase pg_cron):** 8.4 EE service account (JSON key as function secret) → 8.5 daily pg_cron job that `net.http_post`s an Edge Function worker (port of the app's stress-check logic, `@google/earthengine` in Deno — verify import) that writes `alerts_log` and sends Telegram on status change → 8.6 end-to-end test. Full detail in `Backend_Telegram_Roadmap.md`.
