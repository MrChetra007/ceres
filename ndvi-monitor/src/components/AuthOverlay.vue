<template>
  <div id="auth-overlay" class="auth-overlay" v-show="state.authOverlayVisible">
    <div class="auth-card">
      <button class="auth-close" title="Close" @click="store.hideAuthOverlay()"><i class="ti ti-x"></i></button>

      <div class="auth-brand">
        <span class="auth-brand-icon"><i class="ti ti-leaf"></i></span>
      </div>
      <h2>NDVI Rice Monitor</h2>
      <p class="subtitle mono">BATTAMBANG · CAMBODIA</p>
      <p class="auth-tagline">Sign in to sync your saved fields across devices and load live satellite imagery.</p>
      ...
      <div class="auth-fields">
        ...
        <div v-if="!state.eeReady" class="auth-section">
          ...
          <p class="auth-hint">Connects the satellite NDVI imagery</p>
        </div>
      </div>
