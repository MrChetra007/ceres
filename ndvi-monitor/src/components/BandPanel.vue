<template>
  <div class="band-wrap" v-show="state.eeReady">
    <div class="band-panel panel">
      <div
        class="latest-view"
        :class="{ active: !!state.latestView && !state.latestView.noData }"
      >
        <button
          class="latest-view-btn"
          :class="{ loading: state.latestViewLoading }"
          :disabled="state.latestViewLoading"
          @click="store.showLatestView()"
          :title="t('band.latest_tip')"
        >
          <i class="ti ti-satellite"></i>
          <span class="latest-btn-label">{{
            state.latestViewLoading
              ? t("band.latest_loading")
              : t("band.latest_view")
          }}</span>
        </button>
        <span
          class="latest-view-caption"
          v-if="!state.latestViewLoading && state.latestView"
        >
          <template v-if="state.latestView.noData">{{
            t("band.latest_no_data")
          }}</template>
          <template v-else>{{
            t("band.latest_caption", {
              date: state.latestView.date,
              cloud:
                state.latestView.cloudPct != null
                  ? Math.round(state.latestView.cloudPct) + "%"
                  : "—",
            })
          }}</template>
        </span>
      ...
      <div class="segmented" role="group" aria-label="Index">
        ...
        >
          <button
            class="segmented-btn"
            :class="{ active: effectiveIndex() === idx }"
            :data-index="idx"
            @click="store.setIndex(idx)"
          >
            {{ idx === "truecolor" ? t("band.truecolor") : idx.toUpperCase() }}
          </button>
        </Tooltip>
      </div>
      ...
      <div
        class="known-scenes"
        v-show="
          state.currentIndex === 'truecolor' && state.trueColorScenes.length > 1
        "
      >
        ...
        <select
          ...
          :value="
            state.trueColorDate ||
            (state.trueColorScenes[0] && state.trueColorScenes[0].date)
          "
          @change="onTrueColorScene"
          :title="t('band.scene_pick_tip')"
        >
          <option
            v-for="s in state.trueColorScenes"
            :key="s.date"
            :value="s.date"
          >
            {{ sceneOption(s) }}
      ...
      <IndexLegend
        v-if="effectiveIndex() !== 'truecolor'"
        :index="effectiveIndex()"
        compact
      />
      <div
        class="band-base segmented"
        role="group"
        :aria-label="t('band.base_layer')"
      >
        <button
        ...
        >
          {{ b === "street" ? t("topbar.street") : t("topbar.satellite") }}
        </button>
      </div>
      ...
      <div class="areas-group" ref="areasGroupWrap">
        ...
        <div class="band-areas" ref="areasWrap">
          <button
            class="areas-btn"
            :class="{ active: areasOpen || atAreaCap }"
            :title="t('band.switch_area')"
            @click="areasOpen = !areasOpen"
          >
            ...
            <div class="areas-menu" v-show="areasOpen" ref="areasMenuRef">
              ...
              >
                ...
                <span class="areas-item-actions">
                  <i
                    class="ti ti-pencil"
                    :title="t('band.edit_area')"
                    @click.stop="store.openAoiEditorEdit(a.id)"
                  ></i>
                  <i
                    class="ti ti-trash"
                    :title="t('band.delete_area')"
                    @click.stop="remove(a)"
                  ></i>
                </span>
              </button>
              <div class="areas-empty" v-if="state.aois.length === 0">
                {{ t("band.no_areas") }}
              </div>
        ...
        <button
          class="aoi-btn"
          :class="{ active: state.aoiEditorVisible }"
          :title="t('band.edit_area_bounds')"
          @click="editArea"
        >
...
// Follows the ACTIVE (effective) index, not just the user's raw tab pick —
// so the caption stays honest ("RVI · ...") during an auto radar fallback
// instead of continuing to say "NDVI · ..." while radar data is on screen.
const explainerText = computed(() => {
  const cfg = INDICES[effectiveIndex()] || TRUE_COLOR;
  return (
    cfg.name +
    " \u00b7 " +
    (state.preferredLanguage === "km" ? cfg.fullKhm : cfg.full)
  );
});
...
const atAreaCap = computed(
  () => state.aois.length >= state.subscription.maxAois,
);

function displayAoiName(aoi) {
  return aoi.name === "Battambang (default)"
    ? t("band.default_area")
    : aoi.name;
}
...
function onDocClick(e) {
  if (
    areasGroupWrap.value &&
    !areasGroupWrap.value.contains(e.target) &&
