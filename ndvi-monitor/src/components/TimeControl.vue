<template>
  <div
    class="time-panel panel"
    :class="{ loading: state.loading, collapsed }"
    v-show="state.eeReady"
  >
    <div class="time-head">
      <button
        class="play-btn"
        :title="playing ? t('time.playing') : t('time.play')"
        @click="togglePlay"
      >
        <i
          class="ti"
          :class="playing ? 'ti-player-pause' : 'ti-player-play'"
        ></i>
      </button>
      <div
        class="time-title"
        :title="t('time.expand_collapse')"
      ...
      >
        <i
          class="ti"
          :class="collapsed ? 'ti-chevron-up' : 'ti-chevron-down'"
        ></i>
      </button>
      <div class="time-pills">
        ...
        >
          ...
          {{ t("time.jump_last_valid", { date: mainJumpDate }) }}
        </button>
    ...
    <div class="scrubber-wrap">
      <div class="scrubber">
        ...
        <div class="event-markers">
          ...
          <div class="auto-markers-row">
            <div
              ...
              class="auto-marker"
              :class="{
                'auto-dry': state.dryMonthSet.has(i),
                out: !isInRange(i),
              }"
        ...
        <div class="event-legend">
          <span class="ev-key"
            ><i class="ev-dot flood"></i>{{ t("time.flood") }}</span
          >
          <span class="ev-key"
            ><i class="ev-dot drought"></i>{{ t("time.dry_spell") }}</span
          >
          <span class="ev-key"
            ><i class="ev-dot dry-auto"></i
            >{{ t("time.low_rain_chirps") }}</span
          >
        </div>
      ...
      <div v-if="state.compareMode" class="scrubber compare">
        <div class="scrubber-row">
          ...
          <span
            ...
            class="pill cloud-blocked"
            :title="cloudTooltip(state.cloudBlock.right)"
            >☁️ {{ t("time.cloud_blocked") }}</span
          >
          ...
          >
            <i class="ti ti-corner-up-right"></i>
            {{ t("time.jump_last_valid", { date: rightJumpDate }) }}
          </button>
          <ConfidenceBadge
            v-if="!state.loading && rightConf.tier"
            :tier="rightConf.tier"
            :reason="rightConf.reason"
            class="scrubber-conf"
          />
...
const sliderBounds = computed(() =>
  rangeActive.value ? store.sliderBounds() : { min: 0, max: MONTHS.length - 1 },
);
const rangePresetValue = computed(() => {
  if (rangeActive.value) {
    if (
      state.rangePresetId &&
      SEASON_PRESETS.some((p) => p.id === state.rangePresetId)
    )
      return state.rangePresetId;
...
const mainJumpDate = computed(
  () => (state.cloudBlock.main && state.cloudBlock.main.lastValidDate) || "",
);
const rightJumpDate = computed(
  () => (state.cloudBlock.right && state.cloudBlock.right.lastValidDate) || "",
);
...
function cloudTooltip(block) {
  ...
  const pct =
    block.cloudPct != null
      ? Math.round(block.cloudPct) + "% " + t("common.cloud")
      : t("common.cloud_covered");
  const last = block.lastValidDate
    ? t("time.last_valid_reading") + block.lastValidDate
    : t("time.no_cloud_free");
  return (
    t("time.cloud_covered_on") +
    block.month +
    " (" +
    pct +
    ") \u2014 " +
...
function sceneText(count) {
  ...
  const dot = count <= 2 ? "\u25CF " : "";
  return (
    dot + count + " " + (count !== 1 ? t("common.scenes") : t("common.scene"))
  );
}
