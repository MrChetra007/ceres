...
export const INDEX_BANDS: Record<string, { max: number; label: string; phrase: string }[]> = {
  ndvi: [
...
export function translateIndexValue(index: string, value: number): { label: string; phrase: string } {
...
```ts
export function healthScoreWeights(stage: string | null): Record<string, number> {
  switch (stage) {
    case "flooding":
    case "early_growth":
...
```ts
export function detectDiscrepancy(values: Record<string, number>): { message: string } | null {
  // Canopy looks fine (NDVI healthy) but soil moisture is dropping (LSWI low)
  if (values.ndvi >= 0.5 && values.lswi < 0) {
    return { message: "Canopy looks healthy, but soil moisture is dropping — consider irrigation soon." };
  }
  return null;
}
```

Keep this as a short, explicit rules list, not a generic scoring formula — the value of this feature is catching *specific, known-meaningful* contradictions, not flagging every minor numeric difference between indices (which would produce noise, not insight).
